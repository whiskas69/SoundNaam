...
@Route("createPodcast")
public class CreatePodcast extends HorizontalLayout {
    ...
    public CreatePodcast(AudioRepository audioRepository) {
        //in pojo add email and username
        ...
        main.add(textmain,title, description, series, audioFile, uploadAudio, coverFile, uploadCover, submit);
    ...
    private void fetchUserData(String token) {
        ...
        try {
            ...
            Podcast data = new Podcast(null, title.getValue(), description.getValue(), user.getUsername(), audioId, imageId, String.valueOf(new Date()), series.getValue(), 0, 0, 0, user.getEmail(), user.getUsername());
